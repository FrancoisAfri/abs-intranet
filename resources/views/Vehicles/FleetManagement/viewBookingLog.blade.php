                ...
                <div class="box-body">
                    ...
                    <table class="table table-bordered">
                        ...
                        @if (count($vehiclebookinglog) > 0)
                            @foreach ($vehiclebookinglog as $details)
                                <tr id="categories-list">
                                    <td nowrap>
                                        <button details="button" id="edit_compan" class="btn btn-warning  btn-xs"
                                                data-toggle="modal" data-target="#add-booking-modal"
                                                data-id="{{ $details->id }}"><i class="fa fa-eye"></i> view
                                        </button>
                                    </td>
                                <td>{{ !empty($details->require_datetime ) ?  date("F j, Y, g:i a", $details->require_datetime)  : ''}}</td>
                                <td>{{ !empty($details->return_datetime ) ?  date("F j, Y, g:i a", $details->return_datetime)  : ''}}</td>
                                <td>{{ !empty($details->capturer_id) ? $details->capturer_id : ''}}</td>  
                                <td>{{ !empty($details->firstname . ' ' . $details->surname ) ? $details->firstname . ' ' . $details->surname : ''}}</td> 
                                 ...
                                 <td>{{ !empty($details->start_mileage_id) ? $details->start_mileage_id : ''}}</td>
                                 <td>{{ !empty($details->end_mileage_id) ? $details->end_mileage_id : ''}}</td>
                                 <td>{{ !empty($details->start_mileage_id - $details->end_mileage_id ) ? $details->start_mileage_id - $details->end_mileage_id  : ''}}</td>
                                  @if (isset($details) && $details->status === 10)
                            <td>
                                <a href="{{ '/vehicle_management/collect/' . $details->id }}"
                                 id="collect"
                                 class="btn btn-success  btn-xs" data-id="{{ $details->id }}"><i
                                 class="fa fa-handshake-o"></i> collect</a>
                             </td>@elseif (isset($details) && $details->status == 11)
                             <td>
                                <a href="{{ '/vehicle_management/return_vehicle/' . $details->id }}"
                                 id="return"
                                 class="btn btn-info  btn-xs" data-id="{{ $details->id }}"><i
