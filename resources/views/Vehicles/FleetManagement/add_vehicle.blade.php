        ...
        <div class="col-md-12">
            ...
            <!-- /.box-header -->
                            ...
                            @foreach ($vehicledetail as $card)
                                <tr id="categories-list">
                                   <!--  -->
                                    ...
                                    <td>
                                        <!--   leave here  -->
                                        <button vehice="button" id="view_ribbons" class="btn {{ (!empty($card->status) && $card->status == 1) ? " btn-danger " : "btn-success " }}
                                                btn-xs" onclick="postData({{$card->id}}, 'actdeac');"><i class="fa {{ (!empty($card->status) && $card->status == 1) ?
                                      " fa-times " : "fa-check " }}"></i> {{(!empty($card->status) && $vehice->status == 1) ? "De-Activate" : "Activate"}}</button>
                                    </td>
                ...
                <div class="box-footer">
                        <button type="button" class="btn btn-warning pull-left" id="back_button"><i class="fa fa-arrow-left"></i> Back</button>
                      <button type="button" id="cat_module" class="btn btn-warning pull-right" data-toggle="modal" data-target="#add-vehicledetails-modal">Add new Incident Type </button>
...
@section('page_script')
    ...
    <script>
        ...
        $(function () {
           ...
           //      //console.log('strUrl');
           //      var strUrl = '/vehicle_management/add_vehicleDetails';
           //      var modalID = 'add-maintenance-modal';
           //      var objData = {
           //          // vehicle: $('#' + modalID).find('#vehicle').val(),
           //          responsible_for_maintenance: $('#' + modalID).find('#responsible_for_maintenance').val(),
           //          vehicle_make: $('#' + modalID).find('#vehicle_make').val(),
           ...
           //          metre_reading_type: $('#'+modalID).find('input:checked[name = metre_reading_type]').val(),
           ...
           //          title_type: $('#'+modalID).find('input:checked[name = title_type]').val(),
           //          financial_institution: $('#' + modalID).find('#size_of_fuel_tank').val(),
           ...
           //      };
           //      var submitBtnID = 'add_maintenance';
           //      var redirectUrl = '/Jobcard_management/addJob_card';
           //      var successMsgTitle = 'Maintenance Added!';
           //      var successMsg = 'The Maintenance Type has been updated successfully.';
