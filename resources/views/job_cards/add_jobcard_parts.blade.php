            ...
            <div class="box box-warning">
                <div class="box-header with-border">
                        ...
                        <table class="table table-bordered">
                            ...
                            @if (count($jobcartparts) > 0)
                              @foreach ($jobcartparts as $jobparts)
                               ...
                               <td nowrap>
                                        <button vehice="button" id="edit_compan" class="btn btn-warning  btn-xs" data-toggle="modal" data-target="#edit-jobcardparts-modal" data-id="{{ $jobparts->id }}" data-name="{{ $jobparts->name }}" data-description="{{$jobparts->description}}" data-no_of_parts_available="{{$jobparts->no_of_parts_available}}"><i class="fa fa-pencil-square-o"></i> Edit</button>
                                    </td>
                                  ...
                                  <td>
                                    <!--   leave here  -->
                                    <button vehice="button" id="view_ribbons" class="btn {{ (!empty($jobparts->status) && $jobparts->status == 1) ? " btn-danger " : "btn-success " }}
                                      btn-xs" onclick="postData({{$jobparts->id}}, 'actdeac');"><i class="fa {{ (!empty($jobparts->status) && $jobparts->status == 1) ?
                                      " fa-times " : "fa-check " }}"></i> {{(!empty($jobparts->status) && $jobparts->status == 1) ? "De-Activate" : "Activate"}}</button>
                                 </td>
                                 <td><button type="button" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#delete-parts-warning-modal" data-id="{{ $jobparts->id }}"><i class="fa fa-trash"></i> Delete</button></td>
                                </tr>
                                   @endforeach
                               @else
                               <tr id="categories-list">
                    ...
                    <div class="box-footer">
                     ...
                     <button type="button" id="safe_module" class="btn btn-warning pull-right" data-toggle="modal" data-target="#add-jobcardparts-modal">Add new parts</button>
                    </div>
             </div>
        </div>
