        ...
        <div class="col-md-12">
            <div class="box box-warning">
                <div class="box-header with-border">
                    <h3 class="box-title"> Add Process flow</h3>
                    ...
                    <div class="box-body">
                        <table class="table table-bordered">
                            ...
                            @if (count($processflow) > 0)
                              @foreach ($processflow as $service_type)
                               <tr id="categories-list">
                               <td nowrap>
                                        <button vehice="button" id="edit_compan" class="btn btn-warning  btn-xs" data-toggle="modal" data-target="#edit-step-modal"
                                                data-id="{{ $service_type->id }}" data-step_number="{{ $service_type->step_number }}" data-step_name="{{$service_type->step_name}}"
                                                data-jobtitle="{{$service_type->jobtitle}}">
                                            <i class="fa fa-pencil-square-o"></i> Edit</button>
                                    </td>
                                     <td style="text-align: center">{{ (!empty( $service_type->step_number)) ?  $service_type->step_number : ''}} </td>
                                     <td>{{ (!empty( $service_type->step_name)) ?  $service_type->step_name : ''}} </td>
                                     <td>{{ (!empty( $service_type->jobtitle)) ?  $service_type->jobtitle : ''}} </td>
                                  <td>
...
<!--                                 <td><button type="button" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#delete-contact-warning-modal"><i class="fa fa-trash"></i> Delete</button></td>-->
                    ...
                    <div class="box-footer">
                     ...
                     <button type="button" id="safe_module" class="btn btn-warning pull-right" data-toggle="modal" data-target="#add-steps-modal">Add new Process flow</button>
                    </div>
             </div>
        </div>
