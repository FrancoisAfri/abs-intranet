						...
						<table class="table table-bordered">
							...
							@if (count($folders) > 0)
								@foreach ($folders as $folder)
									<tr>
										<td style="text-align: center"><a href="{{ '/dms/folder/view/' . $folder->id}}" class="product-title"><i class="fas fa-folder"></i></a></td>
										<td>{{ (!empty( $folder->folder_name)) ?  $folder->folder_name : ''}} </td>
										<td>{{ (!empty( $folder->description)) ?  $folder->description : ''}} </td>
									</tr>
								@endforeach
							@else
