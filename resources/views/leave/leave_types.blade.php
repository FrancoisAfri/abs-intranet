            ...
            <div class="box-body">
                ...
                <table class="table table-bordered">
                    <tr>
                        ...
                        <th style="width: 40px"></th>
                        {{----}}
                    </tr> @if (count($leave_customs) > 0) @foreach($leave_customs as $leavecustom)
                        <tr id="modules-list">
                            <td nowrap>
                            ...
                            <td>
                                ...
                                <button type="button" id="view_ribbons" class="btn {{ (!empty($leavecustom->status) && $leavecustom->status == 1) ? " btn-danger " : "btn-success " }}
                                        btn-xs" onclick="postData({{$leavecustom->id}}, 'actdeac');"><i class="fa {{ (!empty($leavecustom->status) && $leavecustom->status == 1) ?
...
//
        ...
        $('#update-custom-leave').on('click', function () {
            var strUrl ='/leave/leave_type_edit/' + customleaveId;
            var objData = {
